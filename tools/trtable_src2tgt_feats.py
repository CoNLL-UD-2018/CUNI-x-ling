...
for line in sys.stdin:
    line = line.rstrip('\n')
    if line != '':
        source, tag, feats, target = line.split('\t')
        if (target != ''):
            lsource = source.lower()
            ltarget = target.lower()
            raw_translation_table[(source,)][target] += 1
            raw_translation_table[(lsource,)][ltarget] += 1
            raw_translation_table[(source,tag)][target] += 1
            raw_translation_table[(lsource,tag)][ltarget] += 1
            raw_translation_table[(source,tag,feats)][target] += 1
            raw_translation_table[(lsource,tag,feats)][ltarget] += 1
